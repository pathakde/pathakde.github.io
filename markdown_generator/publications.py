...
def create_md(lines: list, layout: list):
    for item in lines:
        ...
        md_filename = (
            f"{item[layout.index('pub_date')]}-{item[layout.index('url_slug')]}.md"
        )
        html_filename = (
            str(item[layout.index("pub_date")]) + "-" + item[layout.index("url_slug")]
        )
...
def read(filename: str) -> tuple[list, list]:
    ...
    if HEADER_LEGACY == lines[0]:
        ...
        print(lines[0])
        print(
            "The header of the file does not match the expected format", file=sys.stderr
        )
        sys.exit(EXIT_ERROR)
    lines = lines[1:]
